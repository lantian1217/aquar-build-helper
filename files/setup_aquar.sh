...
mkdir -p /opt/aquar/storages/aquar_pool/
echo 'mount nfs'
mount -t nfs $nfspath:/mnt/aquar_pool /opt/aquar/storages/aquar_pool/
if ! grep -q '##\[aquar config start\]##' /etc/fstab;
then
    cat >> /etc/fstab <<EOF
##[aquar config start]##
$nfspath:/mnt/aquar_pool /opt/aquar/storages/aquar_pool nfs defaults,_netdev 0 0
##[aquar config end]##
EOF
else
...
services:
  nextcloud:
    ...
    volumes:
      ...
      - /opt/aquar/storages/aquar_pool:/opt/aquar_pool
    environment:
      - MYSQL_PASSWORD=root
  ...
  jellyfin:
    ...
    environment:
      - TZ=Asia/Shanghai
        # - JELLYFIN_PublishedServerUrl="http://192.168.67.241:8096" #optional
    volumes:
      - /opt/aquar/storages/apps/jellyfin/config:/config
      - /opt/aquar/storages/apps/jellyfin/cache:/cache
      - /opt/aquar/storages/aquar_pool/movies:/media
    restart: unless-stopped
    privileged: true
    devices:
      - /dev/dri:/dev/dri
  ...
  photoprism:
    ...
    volumes:
      - "/opt/aquar/storages/aquar_pool/images:/photoprism/originals"
  ...
  #     - /opt/aquar/storages/aquar_pool:/user-files
  navidrome:
    ...
    volumes:
      ...
      - "/opt/aquar/storages/aquar_pool/music:/music:ro"
  aquarhome:
  ...
  qbittorrent:
    ...
    volumes:
      ...
      - /opt/aquar/storages/aquar_pool/qbdownloads:/downloads
